	...
	if (currentCmd != COMMAND_NO_COMMAND) { // last command doesn't completed
		if (currentCmd != cmd) { //and previous command is differ else nothing to do
			if ((cmd == COMMAND_FORCE_STOP || cmd == COMMAND_STOP) && status!=STATUS_OFF) { // run STOP when Device is not OFF else command is ignored 
				if (cmd == COMMAND_STOP && status == STATUS_ON && lastStatusTimestamp+minTimeOn>Config.counter100) { //wait until minTimeOn
					WaitTime(lastStatusTimestamp + minTimeOn - Config.counter100);
				}
				else {
					DeviceCommand(cmd);
				}
		...
		if (cmd == COMMAND_START && status == STATUS_OFF && lastStatusTimestamp + minTimeOff > Config.counter100) { //wait until minTimeOff
			WaitTime(lastStatusTimestamp + minTimeOff - Config.counter100);
		}
		else {
			DeviceCommand(cmd);
		}
...
void OutputDevice::WaitTime(unsigned long delay)
{
	this->callBack = NULL;
	if (delay > 0) {
		nextDeviceReady = Config.counter100 + delay;
	}
}
